...
batfile_pw = 'HKH_pw.bat'
batfile_wm = 'HKH_wm.bat'
batfile_rgb = 'HKH_rgb.bat'
batfile_rtc = 'HKH_rtc.bat'

vars_pw = [r'%ppath% %mdcspath%\scripts\MDCS.py -i:%mdcspath%\Parameter\Config\pw_mosaic.xml '
           r'-m:%gdbwks%\pw -s:%acspath%%s3tag%\ -p:%cachepath%\$cachelocation '
           r'-p:USE_PIXEL_CACHE$pixelcache -p:%s3tag%$stag -c:CM+AF+AR+UpdateFieldsWM+BF+BB+SP+CC',
           r'%ppath% %mdcspath%\scripts\MDCS.py -i:%mdcspath%\Parameter\Config\pw_overviews.xml '
           r'-m:%gdbwks%\pw -s:%outcrf% -c:SE+CRA+AR+UpdateOverviewFields -p:%outcrf%$outcrf']
...
# run the batch files
subprocess.call([batfile_pw])
print('Perennial water mosaic dataset complete.')
