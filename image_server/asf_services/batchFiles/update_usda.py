...
batfile_wm = 'USDA_wm.bat'
batfile_rgb = 'USDA_rgb.bat'
batfile_rtc = 'USDA_rtc.bat'
...
for bat in batfiles:
    with open(bat, 'w') as f:
        ...
        if bat == 'USDA_wm.bat':
            ...
            for vb in vars_wm:
                print(vb, file=f)
        elif bat == 'USDA_rgb.bat':
            print(crf_rgb, file=f)
            print('\nREM ---------- Using ACS File -------------------------', file=f)
            for vb in vars_rgb:
                print(vb, file=f)
        elif bat == 'USDA_rtc.bat':
...
arcpy.AID.MAIDIS("asf-daac", "Update Service", "test", "None", "USDA_WM", None, aid_wm, "Dedicated Instance",
                 "**TEST OF WATERMAP EXTENT PRODUCT IN A REGION OTHER THAN WHERE IT WAS VALIDATED** "
                 "Watermap Extent products generated from Sentinel-1 SAR imagery over AOIs in China, Madagascar and"
                 "Pakistan, generated by ASF."
                 "This service is for validation only, and not approved for operational use.",
                 "Imagery products processed by ASF DAAC HyP3 2022 using "
                 "GAMMA software. Contains modified Copernicus Sentinel data 2021-2022, processed by ESA.", '',
                 False, False, True, None, None, None, None)
print('Watermap Extent Image Service updated.')
print('Updating RGB Image Service...')
arcpy.AID.MAIDIS("asf-daac", "Update Service", "test", "None", "USDA_RGB", None, aid_rgb, "Dedicated Instance",
                 "Sentinel-1 RGB Decomposition of RTC VV and VH imagery over AOIs in China, Madagascar and"
                 "Pakistan, processed by ASF. "
                 ...
                 "Copernicus Sentinel data 2021-2022, processed by ESA.", '', False, False, True, None, None, None, None)
print('RGB Image Service updated.')
print('Updating RTC Image Service...')
arcpy.AID.MAIDIS("asf-daac", "Update Service", "test", "None", "USDA_RTC", None, aid_rtc, "Dedicated Instance",
                 "Radiometric Terrain Corrected (RTC) products generated from Sentinel-1 SAR imagery over AOIs in "
                 "China, Madagascar and Pakistan, processed by ASF."
                 "Surface water appears very dark under calm conditions, as signal bounces off the "
                 "surface away from the sensor. High VV values are commonly driven by surface roughness and/or "
                 "high soil moisture, and high VH values commonly indicate the presence of vegetation.",
                 "RTC products processed by ASF DAAC HyP3 2022 using GAMMA software. "
