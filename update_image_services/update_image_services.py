import os, arcpy, datetime, boto3
...
class messager:
    ...
    def log(self, msg):
        ...
        self.messagelist.append(msg) ##hold onto message to append to email body
    
    def SaveToTxt(log_dir,messagelist):
...
class mosaicDataset:
    ...
    def build_boundary_mosaic_dataset(self, **kwargs):
        ...
        arcpy.management.BuildBoundary(in_mosaic_dataset = self.mosaic_dataset, \
                                                   **kwargs)
    ...
    def calculate_fields_with_selection(self, where_clause=None, **kwargs):
        ...
        if where_clause:
            ...
            arcpy.management.CalculateFields(in_table = selection, \
                                                **kwargs)
        else:
            arcpy.management.CalculateFields(in_table = self.mosaic_dataset, \
                                    **kwargs)        
    ...
    def remove_rasters_from_mosaic_dataset(self, **kwargs):
        ...
        arcpy.management.RemoveRastersFromMosaicDataset(in_mosaic_dataset = self.mosaic_dataset, \
                                                        **kwargs)
...
class raster:
    ...
    def copy_raster(self, **kwargs):
        ...
        arcpy.management.CopyRaster(in_raster = self.in_raster,\
                                    out_rasterdataset = self.out_rasterdataset,\
                                    **kwargs)
...
class s3_Object:
    def __init__(self, s3_bucket, s3_dir, aws_access_key_id, aws_secret_access_key, time_period_days, ovi_time_period_days, log):
    ...
    def delete_S3_object_by_date(self):
        ...
        for obj in self.s3.list_objects(Bucket=self.s3_bucket, Prefix=self.s3_dir, Delimiter='')['Contents']:
            s3_obj_key = obj['Key']
            if ("tif" in s3_obj_key) and (datetime.datetime.today() - datetime.datetime.strptime(s3_obj_key.split("/")[-1].split("_")[2][0:8],"%Y%m%d")).days >= self.time_period_days:
                print(s3_obj_key)
                raster_del_list.append(s3_obj_key)
                #s3.delete_object(Bucket=s3_bucket, Key=s3_obj_key)
            elif ("crf" in s3_obj_key) and ("_alllayers" in s3_obj_key) and (datetime.datetime.today() - datetime.datetime.strptime(s3_obj_key.split("/")[1].replace(".crf","").replace("Ovi","").replace("_",""),"%Y%m%d")).days >= self.ovi_time_period_days:
                print(s3_obj_key)
                raster_del_list.append(s3_obj_key)
                #s3.delete_object(Bucket=s3_bucket, Key=s3_obj_key)
            elif ("crf" in s3_obj_key) and ("conf" in s3_obj_key) and (datetime.datetime.today() - datetime.datetime.strptime(s3_obj_key.split("/")[1].replace(".crf","").replace("Ovi","").replace("_",""),"%Y%m%d")).days >= self.ovi_time_period_days:
                print(s3_obj_key)
                raster_del_list.append(s3_obj_key)
                #s3.delete_object(Bucket=s3_bucket, Key=s3_obj_key)
...
def main():
    ...
    date_sel = 'StartDate <= timestamp ' + "'" + str((datetime.datetime.today() - datetime.timedelta(days=time_period_days)).strftime("%Y-%m-%d %I:%M:%S"))+ "'"
    # ovi_sel: select the overview files 
    ovi_sel = "Name LIKE '%Ovi%'"
    
    # Log Messages
    logMessages = messager()
    ...
    manage_source = mosaicDataset(source_mds, "Raster Dataset", acs_path_s3, logMessages.log)
    ...
    manage_derived = mosaicDataset(derived_mds, "Table / Raster Catalog", source_mds, logMessages.log)
    ...
    manage_derived.remove_rasters_from_mosaic_dataset(\
                            where_clause = date_sel, \
                            update_boundary = "UPDATE_BOUNDARY" \
                            )

    # Create overview file for the derived mosaic dataset
    manage_derived_create_ovi = raster(derived_mds, overview_crf, logMessages.log)
    manage_derived_create_ovi.copy_raster()
    
    # Add overview file to derived mosaic dataset and calculate fields
    manage_derived = mosaicDataset(derived_mds, "Raster Dataset", overview_crf, logMessages.log)
    manage_derived.add_rasters_to_mosaic_dataset(\
                            update_cellsize_ranges = "", \
                            update_boundary = "UPDATE_BOUNDARY", \
                            update_overviews = "NO_OVERVIEWS", \
    ...
    manage_reference = mosaicDataset(reference_mds, "", "", logMessages.log)
    manage_reference.build_boundary_mosaic_dataset()
    
    # Delete outdated raster files in S3
    #del_s3 = s3_Object(s3_bucket, s3_dir, aws_access_key_id, aws_secret_access_key, time_period_days, ovi_time_period_days, logMessages.log)
    ...
    messager.SaveToTxt(log_dir,logMessages.messagelist)